...
def main():
    ...
    with col2:
        if st.button("🏢 Start Analysis", width='content', type="primary"):
            safe_nav("Retrieve_city_data")

    with col3:
        if st.button("📚 Glossary", width='content'):
            safe_nav("Glossary")

    with col4:
        if st.button("🗺️ City Database", width='content'):
            safe_nav("Database")
    ...
    with f2_col1:
        ...
        * **Circuity & density:** How direct routes are and how tightly intersections pack, across 18 pre-analyzed cities.
        """)
    with f2_col2:
        st.plotly_chart(circuity_chart(), width='content')
